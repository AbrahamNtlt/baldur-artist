<script>
    ...
    const crater = new Array(11).fill(1);
    const rain = new Array(10).fill(1);
</script>
<div class='frame'>
    <div class="moon">
        {#each crater as item, i}
            <div class={`crater-${i +1}`}></div>
        {/each}
    </div>
    ...
    {#each rain as item, i}
        <div class={`drop-big-${i +1}`}></div>
        <div class={`drop-medium-${i +1}`}></div>
        <div class={`drop-small-${i +1}`}></div>
    {/each}
    <div class='front'>
        <div class='temperature'>12°</div>
        <div class='info'>Wind: E 7 km/h <br/> Humidity: 87%</div>
        <table class='preview'>
            <tr>
                <td>Tue</td>
                <td>21° / 9°</td>
